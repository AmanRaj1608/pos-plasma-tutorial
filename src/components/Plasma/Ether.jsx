  ...
  return (
    <div id="PlasmaEther" hidden={selectedToken.label === "Ether" ? false : true}>
      <div className="input-group">
        ...
        <input id="PlasmaEther" type="text" className="form-control" placeholder="value" name="inputValue"
      ...
      <div className="btn-group">
        <button className="btn btn-dark"
          onClick={depositEtherPlasma} disabled={
            Networkid !== 0 && Networkid === config.MATIC_CHAINID
              ? true : false
          }> Deposit</button>

        <button className="btn btn-dark ms-2"
          onClick={burnEtherPlasma} disabled={
            Networkid !== 0 && Networkid === config.ETHEREUM_CHAINID
              ? true : false
          }> burn</button>
        <button className="btn btn-dark ms-2"
          onClick={confirmWithdrawEtherPlasma} disabled={
            Networkid !== 0 && Networkid === config.ETHEREUM_CHAINID
              ? false : true
          }> Confirm Withdraw</button>

        <button className="btn btn-dark ms-2"
          onClick={exitEtherPlasma} disabled={
            Networkid !== 0 && Networkid === config.ETHEREUM_CHAINID
              ? false : true
          }> exit</button>
      </div>
